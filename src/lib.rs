...
pub use opcode::{NoOpOpcode, SingleOpOpcode, TwoOpOpcode};
...
macro_rules! inst {
    (ref, $op1:expr, $op2:expr) => {
        $crate::Instruction::Reference(
            $crate::Operand::Variable($op1.to_string()),
            $op2.to_string(),
        )
    };
    ...
    ($opcode:ident, $op1:expr, $op2:expr) => {
        $crate::Instruction::TwoOp(
            $crate::TwoOpOpcode::$opcode,
            $crate::Operand::Variable($op1.to_string()),
            $crate::Operand::Variable($op2.to_string()),
        )
    };
    ...
    ($opcode:ident, $op:expr) => {
        $crate::Instruction::SingleOp(
            $crate::SingleOpOpcode::$opcode,
            $crate::Operand::Variable($op.to_string()),
        )
    };
    ($opcode:ident, val $op:expr) => {
        $crate::Instruction::SingleOp(
            $crate::SingleOpOpcode::$opcode,
            $crate::Operand::Constant($op),
        )
    };
